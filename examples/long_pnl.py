...
def example_long_pnl():
    ...
    print("Position pnl after open:", int(position["realized_pnl"]) / PRICE_PRECISION)
    ...
    # simulate price change (2x by default)
    price_increase = int(os.getenv("PRICE_INCREASE", 2))
    eth_price_feed.set_latest_answer(
        {
            "positive": True,
            "value": str(start_eth_price * price_increase * ORACLE_PRICE_PRECISION),
        }
    )
    ...
    router.with_wallet(wallets.user0).decrease_position(
        ...
        index_token=contracts["eth"],
        is_long=True,
        collateral_delta=str(amount_in // 2),
        size_delta=str(size_delta // 2),
        price=str(
            start_eth_price * price_increase * PRICE_PRECISION
        ),  # new price is higher
        recipient=wallets.user0.address(),
    )

    # check position decreasing opening
    position = vault.position(
        account=wallets.user0.address(),
        collateral_token=contracts["eth"],
        index_token=contracts["eth"],
    ...
    print(
        "Position pnl after decrease:", int(position["realized_pnl"]) / PRICE_PRECISION
    )
