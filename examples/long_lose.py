...
def set_env():
    ...
    rest_client.send_tokens(
        amount=str(OSMO_SCALE),
        denom="uosmo",
        destination=wallets.user1.address(),
        sender=wallets.deployer,
    ).wait_to_complete()
    ...
    amount_in = 1 * ETH_SCALE
    eth.mint(str(amount_in), wallets.user0.address())
...
def example_long_lose():
    ...
    eth.with_wallet(wallets.user0).increase_allowance(
    ...
    position = vault.position(
        ...
        collateral_token=contracts["eth"],
        index_token=contracts["eth"],
        is_long=True,
    )
    ...
    print("Balances user0 after opening position:", get_balances(wallets.user0))
    print("Balances user1 after opening position:", get_balances(wallets.user1))

    # simulate price change (0.9x by default)
    price_increase = float(os.getenv("PRICE_INCREASE", 0.9))
    eth_price_feed.set_latest_answer(
        {
            "positive": True,
            "value": str(
                start_eth_price * int(price_increase * ORACLE_PRICE_PRECISION)
            ),
        }
    )

    # prepare position liquidator
    vault.set_is_liquidator(wallets.user1.address(), True)

    # liquidate the position
    vault.with_wallet(wallets.user1).liquidate_position(
        account=wallets.user0.address(),
        collateral_token=contracts["eth"],
        index_token=contracts["eth"],
        is_long=True,
        fee_recipient=wallets.user1.address(),
    )

    # check balances after liquidating
    pool_after_liquidate = get_pool_amounts()

    print(
        "Pool amounts after liquidating position:",
        {
            "eth": pool_after_liquidate["eth"] - pool_initial["eth"],
            "usdc": pool_after_liquidate["usdc"] - pool_initial["usdc"],
        },
    )
    print("Balances user0 after liquidating position:", get_balances(wallets.user0))
    print("Balances user1 after liquidating position:", get_balances(wallets.user1))
